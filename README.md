...
udp_bridge file with the format python udp_bridge.py --port COM5 --verbose (change COM5 with whatever port your esp is hooked up to). This will not work if your ESP output is 
in the format of AX:<float> AY:<float> AZ:<float> | GX:<float> GY:<float> GZ:<float>, this is what pyserial looks for to parse. 
